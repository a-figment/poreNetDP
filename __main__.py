...
def update_concentration(sid, inc, graph, edges, data):
    cb, cc, cd = [], [], []
...
probe = Pro.Probe(sid, edges, graph, snode=8, opts="path", total_nodes=10)
...
while t < tmax and i < iters and not state:
    ...
    cb, cc, cd = update_concentration(sid, inc, graph, edges, data)
    ...
    if stop_condition(sid, t, i, iterator_dissolved):
        ...
        if sid.track_type == "dissolved" and iterator_dissolved in sid.track_list:
            ...
            live_tit = f"\n$t =$ {t:.1f}, $Vol. Diss. =$ {data.dissolved_v:.1f}" + \
                    f" Mean $f = ${np.mean(edges.ftrans):.3f}" + \
                    f", Mean $d = ${np.mean(edges.diams):.3f}"
